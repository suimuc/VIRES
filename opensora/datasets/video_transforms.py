...
def resize_crop_mask_to_fill(clip, target_size):
    if not _is_tensor_video_clip(clip):
        raise ValueError("clip should be a 4D torch.tensor")
    h, w = clip.size(-2), clip.size(-1)
    th, tw = target_size[0], target_size[1]
    ...
    if rh > rw:
        sh, sw = th, round(w * rh)
        clip = resize(clip, (sh, sw), "nearest")
        i = 0
        j = int(round(sw - tw) / 2.0)
    else:
        sh, sw = round(h * rw), tw
...
def resize_video(clip, target_size):
    ...
    h, w = clip.size(-2), clip.size(-1)
    th, tw = target_size[0], target_size[1]
    clip = resize(clip, (th, tw), "bilinear")
    return clip
...
class ResizeCropMask:
    ...
    def __call__(self, clip):
        clip = resize_crop_mask_to_fill(clip, self.size)
        return clip
...
class ResizeVideo:
    ...
    def __call__(self, clip):
        clip = resize_video(clip, self.size)
        return clip
