...
class Attention_sketch(nn.Module):
    def __init__(
        self,
        dim: int,
        num_heads: int = 8,
    ...
    def forward(self, x: torch.Tensor, hint, sigma=None) -> torch.Tensor:
        B, N, C = x.shape
        # flash attn is not memory efficient for small sequences, this is empirical
        q = self.q_linear(x)
        k = self.k_linear(hint)
        v = self.v_linear(hint)
        
        qkv_shape = (B, N, self.num_heads, self.head_dim)
        q = q.view(qkv_shape).permute(0,2,1,3)
        k = k.view(qkv_shape).permute(0,2,1,3)
        v = v.view(qkv_shape).permute(0,2,1,3)
        ...
        attn = q @ k.transpose(-2, -1)  # translate attn to float32
        attn = attn.to(torch.float32)
        A = torch.eye(N).unsqueeze(0).unsqueeze(0).to(dtype=attn.dtype, device=attn.device)
        if (sigma is None):
            sigma = torch.zeros((B,), dtype=attn.dtype).to(attn.device)
        w = 2.0 * torch.log(sigma + 1) * attn.max()
        w = w.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to(dtype=attn.dtype, device=attn.device)
        attn = attn + w * A
...
class SeqParallelAttention_sketch(Attention_sketch):
    def __init__(
        self,
        dim: int,
        num_heads: int = 8,
    ...
    def forward(self, x: torch.Tensor, hint, sigma=None) -> torch.Tensor:
        ...
        A = torch.eye(N * sp_size).unsqueeze(0).unsqueeze(0).to(dtype=attn.dtype, device=attn.device)
        if (sigma is None):
            sigma = torch.zeros((B,), dtype=attn.dtype).to(attn.device)
        w = 2.0 * torch.log(sigma + 1) * attn.max()
        w = w.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to(dtype=attn.dtype, device=attn.device)
