...
class Control_Mask_RFlowScheduler:
    def __init__(
        self,
        num_timesteps=1000,
        num_sampling_steps=10,
    ...
    def training_losses(self, model, x_start, model_kwargs=None, noise=None, mask=None, weights=None, t=None, hint = None, video_mask = None):
        """
        Compute training losses for a single timestep.
        Arguments format copied from opensora/schedulers/iddpm/gaussian_diffusion.py/training_losses
        Note: t is int tensor and should be rescaled from [0, num_timesteps-1] to [1,0]
        ...
        t0 = torch.zeros_like(t)
        x_t0 = self.add_noise(x_start, noise, t0)
        x_t = x_t * video_mask + x_t0 * (1. - video_mask)
        if mask is not None:
            x_t = torch.where(mask[:, None, :, None, None], x_t, x_t0)
            
        x_t_with_mask = torch.cat([x_t, video_mask], dim = 1)
            

        terms = {}
        model_output = model(x_t_with_mask, hint, t, **model_kwargs)
        velocity_pred = model_output.chunk(2, dim=1)[0]
        velocity_pred = velocity_pred[:, : 4]
        
        if weights is None:
            loss = mean_flat(((velocity_pred - (x_start - noise)) * video_mask).pow(2), mask=mask)
        else:
            weight = _extract_into_tensor(weights, t, x_start.shape)
            loss = mean_flat(weight * ((velocity_pred - (x_start - noise)) * video_mask).pow(2), mask=mask)
        terms["loss"] = loss
