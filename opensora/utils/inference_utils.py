...
def read_mask(video_path):
    ...
    while cap.isOpened():
        ...
        frame = cv2.resize(frame, (w // re, h // re), interpolation=cv2.INTER_NEAREST).astype(np.float32)
        # Gauss
        frame = cv2.GaussianBlur(frame, (5, 5), 4)
        frame = cv2.resize(frame, (w, h), interpolation=cv2.INTER_NEAREST).astype(np.float32)


        frame = torch.from_numpy(frame)
        frame_ori = torch.from_numpy(frame_ori)
        frame = frame.unsqueeze(0).unsqueeze(0)
        frame_ori = frame_ori.unsqueeze(0).unsqueeze(0)

        frames.append(frame)
        frames_ori.append(frame_ori)
    cap.release()
    return torch.cat(frames), torch.cat(frames_ori), frame_rate

def down_sample_mask(video_mask, micro_frame_size = 17, down_time = 4, down_height = 8, down_width = 8):
    
    video_mask_ds = []
    
    for i in range(0, video_mask.shape[2], micro_frame_size):
        video_mask_bs = video_mask[:, :, i : i + micro_frame_size]
        video_mask_bs_1 = video_mask_bs[:, :,0]
        video_mask_bs_1 = torch.nn.functional.interpolate(video_mask_bs_1, size=(video_mask_bs.shape[3]//down_height,video_mask_bs.shape[4]//down_width), mode='bilinear', align_corners=False)
        video_mask_ds.append(video_mask_bs_1.unsqueeze(2))
        video_mask_bs_res = video_mask_bs[:, :,1:]
        video_mask_bs_res = torch.nn.functional.interpolate(video_mask_bs_res, size=(video_mask_bs.shape[2]//down_time,video_mask_bs.shape[3]//down_height,video_mask_bs.shape[4]//down_width), mode='trilinear', align_corners=False)
        video_mask_ds.append(video_mask_bs_res)
        
    video_mask_ds = torch.cat(video_mask_ds, dim = 2)
    video_mask_ds = video_mask_ds > 0
    return video_mask_ds
