...
import torch
from opensora.datasets.hed import HEDdetector
...
def main():
    ...
    # sketch_processor
    preprocessor_sketch = HEDdetector()
    preprocessor_sketch.netNetwork = preprocessor_sketch.netNetwork.to(device, torch.float)
    ...
    model = (
        build_module(
            ...
            in_channels=vae_out_channels + 1,
    ...
    if dist.get_rank() == 0:
        print("These para are training")
        print(parameters_name)
    # == setup optimizer ==
    optimizer = HybridAdam(
        filter(lambda p: p.requires_grad, selected_parameters),
        adamw_mode=True,
        lr=cfg.get("lr", 1e-4),
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                with timers["move_data"] as move_data_t:
                    ...
                    y = batch.pop("text")
                    video_mask = batch.pop("video_mask").to(device, dtype) # [B, C, T, H, W]
                    hint = batch.pop("edge").to(device, torch.float)  # [B, T, C, H, W]
                    tmp = []
                    for i in range(len(hint)):
                        with torch.no_grad():
                            clip = hint[i]
                            sketch = preprocessor_sketch(clip)
                            tmp.append(sketch.unsqueeze(0))
                    del hint
                    hint = torch.cat(tmp, dim=0)
                    hint = hint.permute(0, 2, 1, 3, 4).to(device, dtype)
                    hint = hint * video_mask
                    hint = hint / 255.
                    del tmp
                    video_mask_ds = []
                    for i in range(0, video_mask.shape[2], vae.micro_frame_size):
                        video_mask_bs = video_mask[:, :, i: i + vae.micro_frame_size]
                        video_mask_bs_1 = video_mask_bs[:, :, 0]
                        video_mask_bs_1 = torch.nn.functional.interpolate(video_mask_bs_1, size=(video_mask_bs.shape[3] // 8, video_mask_bs.shape[4] // 8), mode='bilinear', align_corners=False)
                        video_mask_ds.append(video_mask_bs_1.unsqueeze(2))
                        video_mask_bs_res = video_mask_bs[:, :, 1:]
                        video_mask_bs_res = torch.nn.functional.interpolate(video_mask_bs_res, size=(video_mask_bs.shape[2] // 4, video_mask_bs.shape[3] // 8, video_mask_bs.shape[4] // 8),mode='trilinear', align_corners=False)
                        video_mask_ds.append(video_mask_bs_res)

                    # import pdb;pdb.set_trace()
                    video_mask_ds = torch.cat(video_mask_ds, dim=2)
                    video_mask_ds = video_mask_ds > 0
                    video_mask_ds = video_mask_ds.to(device, dtype)
                    del video_mask
                ...
                with timers["diffusion"] as loss_t:
                    loss_dict = scheduler.training_losses(model, x, model_args, mask=mask, hint=hint, video_mask=video_mask_ds)
                if record_time:
